/* OnTrack — public site, member area, curator area and admin. No framework, no build step. */
(function () {
  'use strict';
  ...
  function studioDrafts() {
    ...
    for (const id in drafts) {
      try {
        ...
        const e = ExerciseLibrary.get(id) || (d.entry ? OnTrackCatalog.buildFile({ ...(d.group || {}), moves: [d.entry] }, 'draft', OnTrackCatalog.data, true)[0] : ExerciseLibrary.define((k) => MoveSpec.compile(d, k)));
  ...
  const coachEx = (id) => OnTrackCoach.exercises.find(e => e.id === id);

  /* Every move is in the library; the vetted ones (hand-checked rep by rep) simply sort first. */
  /* What browsing offers. A move the library hides (`listed: false`, and unstated that follows
     `vetted`) is still there — a routine that names it runs, a link to it opens — it is simply not
     in the lists, so the moves someone has actually checked are what they meet first. */
  const visibleEx = () => (exercises || []).filter((e) => e.listed !== false);
  /* What the camera does with a move: coaches form, counts reps, or nothing (logged by hand).
     It reads as one more fact about the move, so it sits in the meta line with the rest. */
  ...
  const trackWord = (ex) => `<span class="track t-${trackInfo(ex)[0]}" title="${esc(trackInfo(ex)[2])}">${trackInfo(ex)[1]}</span>`;
  ...
  function optionSummary(ex, o) { const parts = [ex.type === 'reps' ? `${o.target} reps` : `${o.target} s hold`]; if (o.rom) parts.push(`${o.rom}° target`); if (o.variant) { const vo = (ex.options || []).find(x => x.key === 'variant'); parts.push((vo && vo.labels && vo.labels[o.variant]) || o.variant); } if (o.weight && o.weight !== 'none' && o.weight !== 'custom') parts.push(o.weight + ' kg'); if (o.band && o.band !== 'none') parts.push(o.band + ' band'); else if (o.band === 'none' && (ex.options || []).some(x => x.key === 'band' && x.default !== 'none')) parts.push('no band'); if (o.side) parts.push(o.side === 'both' ? 'both sides' : (sideLabel(ex, o.side) || o.side + ' side')); if (o.sets > 1) parts.push(`${o.sets} sets${o.rest ? ', ' + o.rest + ' s rest' : ''}`); return parts.join(' · '); }
  ...
  const heroMark = () => `<span class="wordmark hero-wordmark" role="img" aria-label="OnTrack">OnTrack</span>`;
  ...
  async function renderHome() {
    ...
    render(`<div class="stack">
      ...
      <section class="home-sec"><h2>Moves</h2><p class="muted">Pick one, put the phone down, follow the voice.</p>${listHtml(exs)}<p class="muted search-none" hidden>No move matches that search.</p></section>
  ...
  function exRow(ex) { return `<a class="ex-row" href="#/exercise/${ex.id}" data-name="${esc(ex.name.toLowerCase())}" data-group="${esc(ex.group)}" data-track="${esc(ex.tracking || 'form')}"><span class="glyph">${OnTrackCoach.thumb(coachEx(ex.id))}</span><span class="body"><span class="name">${esc(ex.name)}</span><span class="meta">${ex.type === 'reps' ? 'reps' : 'timed hold'}${ex.tracking === 'none' ? '' : ` · ${ex.view === 'front' ? 'front-on' : 'side-on'}`}<span class="nb"> · ${trackWord(ex)}</span></span></span>${allFree() ? '' : tierBadge(ex)}</a>`; }
  ...
  let navQuery = '';
  /* What the camera can do with a move is the first thing most people want to narrow by — "show me
     only the ones it judges". Kept in the row's own data, so the filter is the same pass as the
     search and nothing has to be re-rendered. */
  let trackFilter = (() => { try { return localStorage.getItem('fz.track') || 'all'; } catch { return 'all'; } })();
  /* With one tier in the list there is nothing to filter, and a row of buttons that all say the
     same number is noise — so the bar only appears when the shortlist actually spans tiers. */
  function filterBar(list) {
    const n = (t) => list.filter((e) => (e.tracking || 'form') === t).length;
    if (new Set(list.map((e) => e.tracking || 'form')).size < 2) return '';
    const opts = [['all', 'All', list.length], ['form', 'Form coached', n('form')], ['reps', 'Counts reps', n('reps')], ['none', 'Guide only', n('none')]];
    return `<div class="ex-filter" id="ex-filter"><span class="k">The camera</span>${opts.map(([v, label, c]) =>
      `<button type="button" data-track="${v}" aria-pressed="${trackFilter === v}" title="${esc((TRACK[v] || [])[2] || 'Every move in the library')}">${esc(label)}<span class="n">${c}</span></button>`).join('')}</div>`;
  }
  function applySearch(root) {
    const s = navQuery.trim().toLowerCase();
    root.querySelectorAll('.ex-row[data-name]').forEach(t => t.hidden = (trackFilter !== 'all' && t.dataset.track !== trackFilter) || (!!s && !t.dataset.name.includes(s) && !t.dataset.group.toLowerCase().includes(s)));
    root.querySelectorAll('.ex-group').forEach(g => g.hidden = ![...g.querySelectorAll('.ex-row')].some(t => !t.hidden));
    const none = root.querySelector('.search-none'); if (none) { none.hidden = !!root.querySelector('.ex-row:not([hidden])'); none.textContent = s ? 'No move matches that search.' : 'No move in the library the camera does that with.'; }
    const bar = root.querySelector('#ex-filter');
    if (bar && !bar.dataset.wired) {
      bar.dataset.wired = '1';
      bar.onclick = (e) => { const b = e.target.closest('button[data-track]'); if (!b) return;
        trackFilter = b.dataset.track; try { localStorage.setItem('fz.track', trackFilter); } catch { }
        bar.querySelectorAll('button').forEach((x) => x.setAttribute('aria-pressed', x === b)); applySearch(root); };
    }
  }
  ...
  $('btn-how').onclick = () => showIntro(true);
  ...
  function exCard(ex) { return `<a class="ex-card" href="#/exercise/${ex.id}"><span class="glyph">${OnTrackCoach.thumb(coachEx(ex.id))}</span><span><span class="name">${esc(ex.name)}</span><br><span class="sum">${esc(ex.summary)}</span></span>${tierBadge(ex)}</a>`; }
  ...
  async function renderExercises() { await refreshMe(); await loadExercises(); /* entitlements can change between views (purchase, curator send) */ const list = visibleEx();
    render(`<div class="stack"><h1>Moves</h1><p class="muted">${allFree() ? 'Pick one, put the phone down, follow the voice.' : ent && ent.tier !== 'anon' && ent.tier !== 'free' ? 'Everything is unlocked on your plan.' : 'Free ones need no account. Pro ones unlock with a subscription or a curator-sent routine.'} Some moves the camera can only count, and some it cannot see at all — each row says which.</p>
      ${filterBar(list)}${listHtml(list)}<p class="muted search-none" hidden>No move matches that search.</p>${ent && !ent.pro && !allFree() ? upgradeCard('Unlock the full library') : ''}</div>`); wireSearch(view); }
  async function renderExercise(id, sub) {
    ...
    render(`<div class="stack">
      ...
      <div class="ex-head"><div><span class="eyebrow">${esc(cat.group)} · ${cat.type === 'reps' ? (cat.repHold ? `reps, held ${cat.repHold} s at the top` : 'reps') : 'timed hold'}${cat.level ? ' · ' + esc(cat.level) : ''}<span class="nb"> · ${trackWord(cat)}</span></span><h1>${esc(cat.name)} <button type="button" class="info-btn" id="do-details" aria-haspopup="dialog" aria-controls="ex-details" aria-label="Details: how it is measured, set-up and form, what goes wrong, dosage" title="Details">i</button> ${allFree() ? '' : tierBadge(cat)}</h1></div>
      ...
      <div class="ex-grid">
        <div class="stack">
          <div class="card move-card"><h3>The move</h3>${OnTrackCoach.phoneInset(ex)}<div id="demo-host">${OnTrackCoach.demo(cat)}</div><div class="fig-toggle" role="group" aria-label="Animation style">${['lines', 'muscles'].map(v => `<button type="button" class="chip" data-fig="${v}" aria-pressed="${(OnTrackCoach.settings.figure || 'lines') === v}">${v === 'lines' ? 'Stick figure' : 'Muscles'}</button>`).join('')}</div></div>
          <div class="card"><h3>Where to put the phone</h3>${OnTrackCoach.cameraDiagram(ex)}<p style="margin-top:8px">${esc(cat.setup)}</p></div>
        </div>
        <div class="stack">
    ...
    wireChips(view, {}, opts);
    OnTrackCoach.mountFigures(view);   /* the stick figure is SVG and needs nothing; the muscle one is a canvas */
    /* the same choice as Settings → "The move's animation", here where the animation is */
    view.querySelectorAll('[data-fig]').forEach(b => { b.onclick = () => { OnTrackCoach.setSetting('figure', b.dataset.fig); view.querySelectorAll('[data-fig]').forEach(x => x.setAttribute('aria-pressed', String(x.dataset.fig === b.dataset.fig))); $('demo-host').innerHTML = OnTrackCoach.demo(cat); OnTrackCoach.mountFigures(view); }; });
  ...
  function bubble(cat, opts, key, values, o = {}) {
    const texts = bubbleTexts(cat, key, values, o); let i = values.indexOf(opts[key]);
    /* a weight the person typed is not one of the steps: it sits on the Other… step, showing its number */
    if (i < 0 && o.custom && Number.isFinite(+opts[key]) && +opts[key] > 0) { i = values.indexOf('custom'); texts[i] = esc(opts[key] + ' ' + (o.customUnit || '')); }
    if (i < 0) i = 0;
    ...
    return `<span class="knob"><span class="k">${esc(label)}</span><button type="button" class="bubble" data-optkey="${key}" data-i="${i}" data-value="${esc(values[i])}" data-values='${esc(JSON.stringify(values))}' data-texts='${esc(JSON.stringify(texts))}' data-titles='${esc(JSON.stringify(titles))}' ${o.custom ? `data-custom="${esc(o.customUnit || '')}"` : ''} aria-label="${esc(label)}, tap to change" title="${esc(titles[i])}">${texts[i]}</button></span>`;
  }
  ...
  function wireChips(root, bags, loose, onChange) {
    root.querySelectorAll('.bubble[data-optkey]').forEach(b => {
      ...
      b.onclick = () => {
        ...
        if (vv === 'custom' && b.dataset.custom !== undefined) typeOwn(b, bag, values, texts, () => { if (onChange) onChange(host, bag); });
        if (onChange) onChange(host, bag);
      };
    });
  }
  /* The Other… step of a weight bubble: a box appears beside it for the number. A valid number
     becomes the value and stays on the bubble; an empty box falls back to the first step. */
  function typeOwn(b, bag, values, texts, done) {
    const unit = b.dataset.custom; const key = b.dataset.optkey; const old = b.parentElement.querySelector('.bubble-in'); if (old) old.remove();
    const inp = document.createElement('input'); inp.type = 'number'; inp.className = 'bubble-in'; inp.min = '0.5'; inp.step = '0.5'; inp.placeholder = unit; inp.setAttribute('aria-label', `${bubbleLabelText(b)} in ${unit}`); inp.inputMode = 'decimal';
    b.after(inp); inp.focus();
    let settled = false;
    const settle = () => {
      if (settled) return; settled = true; const n = +inp.value; inp.remove();
      if (Number.isFinite(n) && n > 0) { bag[key] = n; b.dataset.value = n; b.innerHTML = esc(n + ' ' + unit); b.title = n + ' ' + unit; }
      else { bag[key] = values[0]; b.dataset.i = 0; b.dataset.value = values[0]; b.innerHTML = texts[0]; }
      done();
    };
    inp.onblur = settle; inp.onkeydown = (e) => { if (e.key === 'Enter' || e.key === 'Escape') { e.preventDefault(); settle(); } };
  }
  const bubbleLabelText = (b) => { const k = b.parentElement.querySelector('.k'); return k ? k.textContent : b.dataset.optkey; };
  ...
  function runSequence(rawSteps, { file = null, back = '#/' } = {}) {
    ...
    const startSet = () => {
      ...
      OnTrackCoach.start({ exercise: step.ex, target, options: { ...o, set: setNo }, file, keepCameraAfter: more, exit: (dest) => { close(); location.hash = dest || back; }, done: onDone });
    };
    ...
    const manualSet = () => {
      showReview(true); OnTrackCoach.endRest();   /* a held camera from the previous set is not needed here */
      ...
      if (hold) { portal.querySelector('#man-timer').onclick = (e) => { const b = e.currentTarget; if (timer) return; b.disabled = true; OnTrackCoach.voice.say('Hold.', { priority: 2 }); let left = target;
          timer = setInterval(() => { left--; n = target - left; const el = portal.querySelector('#man-clock'); if (el) el.textContent = left; if (left <= 3 && left > 0) OnTrackCoach.voice.beep(660, 0.06); if (left <= 0) { clearInterval(timer); OnTrackCoach.voice.beep(880, 0.15); finish(); } }, 1000); }; }
    ...
    const restOnCamera = ({ title, review, note, goLabel, onGo }) => {
      ...
      const stop = () => { clearInterval(restTimer); OnTrackCoach.endRest(); showReview(); savePanel(); };
      const go = () => { clearInterval(restTimer); onGo(); };
      const paint = () => OnTrackCoach.restOverlay({ title, text: c.text, checks: c.checks, count: left, note, actions: [{ label: goLabel, fn: go }, { label: 'Stop here', cls: 'ghost', fn: stop }] });
      paint();
      restTimer = setInterval(() => { left--; paint(); if (left <= 3 && left > 0) OnTrackCoach.voice.beep(660, 0.06); if (left <= 0) go(); }, 1000);
    };
    const restScreen = (review, held) => {
      OnTrackCoach.voice.say(`Set ${setNo} done. Rest ${rest} seconds.`, { priority: 2 });
      ...
      restTimer = setInterval(() => { left--; const el = portal.querySelector('#rest-left'); if (el) el.textContent = left; const tp = topBar.querySelector('#rest-left-top'); if (tp) tp.textContent = left + ' s'; if (left <= 3 && left > 0) OnTrackCoach.voice.beep(660, 0.06); if (left <= 0) go(); }, 1000);
    };
    ...
    const nextStepPanel = (review, held) => {
      ...
      OnTrackCoach.voice.say(switching ? `Now the other side. ${nextSide}. Rest ${rest} seconds.` : `${step.ex.name} done. Next up, ${next.ex.name}. Rest ${rest} seconds.`, { priority: 2 });
  ...
  async function renderCurators() {
    ...
    render(`<div class="stack"><div><span class="eyebrow">Directory</span><h1>Find a curator</h1><p class="muted">Physiotherapists and trainers who build routines in ${esc(env.appName || 'OnTrack')} and coach through it. Request a connection; once they accept, routines they send you are unlocked at no extra cost, and they can see how your sets went.</p></div>
  ...
  async function renderCuratorPublic(id) {
    ...
    render(`<div class="stack" style="max-width:720px"><div class="row"><a class="btn ghost small" href="#/curators">← Directory</a></div>
      ...
      <p class="muted" style="font-size:.85rem">${esc(env.appName || 'OnTrack')} does not employ curators or verify treatment; a verified badge means we checked a registration number, nothing more. Fees, if any, are agreed between you and the curator.</p></div>`);
  ...
  async function renderPricing() {
    ...
    render(`<div class="stack"><div><span class="eyebrow">Pricing</span><h1>Free, Pro, Curator</h1><p class="muted">Prepaid periods, no auto-renewal — you'll be reminded before it ends. Prices in INR, GST included.</p></div>
      ...
      <p class="muted" style="font-size:.85rem">Payments are processed by Razorpay; ${esc(env.appName || 'OnTrack')} never sees your card details. ${env.payments === 'mock' ? '<strong>Test mode:</strong> no money moves — the plan activates immediately.' : ''}</p></div>`);
    ...
    const draw = () => {
      render(`<div class="stack"><div class="row"><a class="btn ghost small" href="#/build">← My routines</a></div><h1>${r ? 'Edit routine' : 'New routine'}</h1>
        ...
        <div class="card"><h3>Add exercise</h3>${(() => { const list = visibleEx(); const groups = []; for (const e of list) { let g = groups.find(x => x.name === e.group); if (!g) groups.push(g = { name: e.group, items: [] }); g.items.push(e); }
          return groups.map(g => `${groups.length > 1 ? `<p class="meta" style="margin-top:8px">${esc(g.name)}</p>` : ''}<div class="row">${g.items.map(ex => `<button class="btn ghost small" data-add="${ex.id}" title="${esc(trackInfo(ex)[1])}">${esc(ex.name)}${ex.tracking === 'none' ? ' <span class="muted">✎</span>' : ''}</button>`).join('')}</div>`).join(''); })()}</div>
  ...
  function itemEditorHtml(it, i, n) {
    ...
    return `<div class="card editor-item" data-i="${i}"><div class="head"><h3>${esc(ex.name)}</h3><span class="badge">${ex.type === 'reps' ? 'reps' : 'hold'}${ex.tracking === 'none' ? '' : ` · ${ex.view === 'front' ? 'front-on' : 'side-on'}`}</span>${trackWord(ex)}<button class="btn ghost tiny it-up" ${i === 0 ? 'disabled' : ''}>↑</button><button class="btn ghost tiny it-down" ${i === n - 1 ? 'disabled' : ''}>↓</button><button class="btn ghost tiny it-remove">Remove</button></div>
  ...
  async function renderAccount() {
    ...
    render(`<div class="stack" style="max-width:640px"><h1>Account</h1>
      ...
      <div class="card"><h3>Coach settings (this device)</h3><div class="form cols"><label class="field"><span>Pose model</span><select id="cs-model"><option value="lite">Lite (fast)</option><option value="full">Full (precise)</option></select></label><label class="field"><span>Smoothing</span><select id="cs-smooth"><option value="low">Low</option><option value="med">Medium</option><option value="high">High</option></select></label><label class="field"><span>Voice cues</span><select id="cs-voice"><option value="on">On</option><option value="off">Off</option></select></label><label class="field"><span>Keep the set&rsquo;s video</span><select id="cs-video"><option value="on">On — watch it back</option><option value="off">Off — skeleton only</option></select></label><label class="field"><span>The move’s animation</span><select id="cs-figure"><option value="lines">Stick figure (default)</option><option value="muscles">Anatomical, with the muscles working</option></select></label><label class="field"><span>Head on the skeleton</span><select id="cs-head"><option value="">Default</option><option value="ball">Solid ball</option><option value="circle">Outlined circle</option><option value="dot">Small dot</option><option value="face">Nose and ears</option><option value="none">No head</option></select></label><label class="field"><span>Your height</span><select id="cs-height">${heightOptions()}</select></label><label class="field"><span>Voice</span><select id="cs-voicename"><option value="auto">Auto (most natural available)</option></select></label><label class="field"><span>Speaking speed</span><select id="cs-voicerate"><option value="slow">Slower</option><option value="normal">Normal</option><option value="fast">Faster</option></select></label></div><div class="row" style="margin-top:8px"><button class="btn ghost small" id="cs-test">Hear this voice</button><span class="muted" id="cs-voicenote" style="font-size:.85rem"></span></div></div>
  ...
  function renderSettings() {
    render(`<div class="stack" style="max-width:640px"><h1>Settings</h1>
      <div class="card"><h3>Coach (this device)</h3><div class="form cols"><label class="field"><span>Pose model</span><select id="cs-model"><option value="lite">Lite (fast)</option><option value="full">Full (precise)</option></select></label><label class="field"><span>Smoothing</span><select id="cs-smooth"><option value="low">Low</option><option value="med">Medium</option><option value="high">High</option></select></label><label class="field"><span>Voice cues</span><select id="cs-voice"><option value="on">On</option><option value="off">Off</option></select></label><label class="field"><span>Keep the set&rsquo;s video</span><select id="cs-video"><option value="on">On — watch it back</option><option value="off">Off — skeleton only</option></select></label><label class="field"><span>The move’s animation</span><select id="cs-figure"><option value="lines">Stick figure (default)</option><option value="muscles">Anatomical, with the muscles working</option></select></label><label class="field"><span>Head on the skeleton</span><select id="cs-head"><option value="">Default</option><option value="ball">Solid ball</option><option value="circle">Outlined circle</option><option value="dot">Small dot</option><option value="face">Nose and ears</option><option value="none">No head</option></select></label><label class="field"><span>Your height</span><select id="cs-height">${heightOptions()}</select></label><label class="field"><span>Voice</span><select id="cs-voicename"><option value="auto">Auto</option></select></label><label class="field"><span>Speaking speed</span><select id="cs-voicerate"><option value="slow">Slower</option><option value="normal">Normal</option><option value="fast">Faster</option></select></label></div><div class="row" style="margin-top:8px"><button class="btn ghost small" id="cs-test">Hear this voice</button><span class="muted" id="cs-voicenote" style="font-size:.85rem"></span></div></div>
      <div class="card"><h3>How it works</h3><p class="muted">The three steps of a session: position the camera, follow the voice, review the set.</p><div class="row" style="margin-top:8px"><button class="btn ghost small" id="cs-intro">Show me again</button></div></div>
  ...
  /* 4'6" to 7'0" an inch at a time, each with its centimetres; the value is inches */
  const heightText = (inches) => `${Math.floor(inches / 12)}'${inches % 12}" (${Math.round(inches * 2.54)} cm)`;
  function heightOptions() { let out = ''; for (let n = 54; n <= 84; n++) out += `<option value="${n}">${heightText(n)}</option>`; return out; }
  function wireCoachSettings() {
    for (const k of ['model', 'smooth', 'voice', 'video', 'head', 'figure']) { const el = $('cs-' + k); if (!el) continue; el.value = OnTrackCoach.settings[k]; el.onchange = () => OnTrackCoach.setSetting(k, el.value); }
    const hSel = $('cs-height'); if (hSel) { hSel.value = String(OnTrackCoach.settings.heightIn || 71); hSel.onchange = () => OnTrackCoach.setSetting('heightIn', +hSel.value); }
    const rateSel = $('cs-voicerate'); if (rateSel) { rateSel.value = OnTrackCoach.settings.voiceRate || 'normal'; rateSel.onchange = () => { OnTrackCoach.setSetting('voiceRate', rateSel.value); OnTrackCoach.voice.preview(); }; }
    const fillVoices = () => { const sel = $('cs-voicename'); if (!sel) return; const cur = OnTrackCoach.settings.voiceName || 'auto'; const auto = OnTrackCoach.pickVoice(); const note = $('cs-voicenote'); if (note) note.textContent = auto ? `Using ${auto.name}${auto.localService ? '' : ' (needs a connection)'}` : 'No speech voice on this device'; sel.innerHTML = `<option value="auto">Auto${auto ? ' — ' + esc(auto.name) : ''}</option>` + OnTrackCoach.listVoices().map(v => `<option value="${esc(v.name)}" ${v.name === cur ? 'selected' : ''}>${esc(v.name)} (${esc(v.lang)})</option>`).join(''); sel.value = cur; };
    /* the list arrives asynchronously on Chrome and Android, so it is refilled when it does */
    fillVoices(); setTimeout(fillVoices, 400); setTimeout(fillVoices, 1500);
    try { speechSynthesis.addEventListener('voiceschanged', fillVoices); } catch { }
    $('cs-voicename').onchange = () => { OnTrackCoach.setSetting('voiceName', $('cs-voicename').value); fillVoices(); OnTrackCoach.voice.preview(); };
    $('cs-test').onclick = () => OnTrackCoach.voice.preview();
  }
  /* ================= CURATOR ================= */
