...
(function () {
  ...
  function specToEntry(s) {
    ...
    put('faults', (s.faults || []).map((f) => {
      ...
      const maybe = f.tentative ? { tentative: true } : {};
      if (f.rule) return { ...base, ...maybe, rule: f.rule, ...(f.rule === 'fast' ? { minMs: f.minMs } : f.rule === 'return' && Number.isFinite(f.threshold) ? { threshold: f.threshold } : {}) };
      if (!tracked || !isLive(f)) return base;
      const o = { ...base, ...maybe, metric: f.metric }; if (f.rel === 'change') o.rel = 'change'; o.op = f.op; o.threshold = f.threshold;
  ...
  const TENTATIVE_HELP = 'Held back when the camera can see the thing but cannot swear to it — a reading near its own noise, a landmark the model half guesses. It is still measured, still counted and still attached to the reps it happened on; it just never speaks, and it costs nothing off the score. Mid-rep a cue is an instruction, and an instruction you are not sure of is worse than silence. It appears after the set under "worth a look", where the person can watch the rep back and say yes or no.';
  ...
  function faultCard(s, f, i) {
    const isRule = ['shallow', 'fast', 'return', 'shortHold'].includes(f.rule);
  ...
  function wireFaults(s) {
    ...
    if ($('add-hold')) $('add-hold').onclick = () => { s.faults.push({ id: 'hold', rule: 'shortHold', label: 'No pause at the top', cue: 'Hold it there', tip: 'The pause at the top is where the muscle works hardest. A rep that comes straight back down counts as a partial.', severity: 1 }); saveState(); render(); };
