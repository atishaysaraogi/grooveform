...
- The same keypoint recording is what the review's **Watch it back** plays: `client/coach/replay.js` draws the skeleton per frame (smoothed as the coach drew it live, with the same unsure-joints-hidden rule) with a timeline of reps, fault spans and cues under it, and can hand the person the set as a self-contained HTML report (recording and player inlined) or a video of the playback — MP4/H.264 where the browser can write one (Chrome 126+, Safari, Edge), WebM where it cannot (Firefox), the file named for whichever came back. It loads nothing from the app so the report opens anywhere. Both the player and the WebM run the skeleton off the wall clock, because MediaRecorder stamps every frame it is handed by that clock and the set's own video plays underneath on it: counting rendered frames instead let the skeleton fall behind whenever the drawing was slower than real time (one set took 74.8 s to draw 63.3 s, and the picture finished eleven seconds ahead of the figure on it). Drawn slower than real time the export drops frames; the video underneath is put back whenever it has slipped more than `VIDEO_SLIP`. A video file being analysed is attached to the recording as it is (it is the set's video already; nothing is captured), with time 0 of the recording placed wherever the file had got to on its first analysed frame, so the replay and the export get the picture there too. Neither a captured nor an attached video ever enters the diagnostics JSON.
