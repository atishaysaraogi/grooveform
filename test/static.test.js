...
srv.listen(0, async () => {
  ...
  const contrast = `(() => {
    ...
    const at = (sel, against) => { const el = document.querySelector(sel); return el ? ratio(getComputedStyle(el).color, against || bg) : null; };
    // The coach overlay sits on its own card, so measure its text against that card, not the page.
    document.getElementById('coach').hidden = false;
    document.getElementById('screen-live').classList.add('active');
    document.getElementById('overlay-msg').hidden = false;
